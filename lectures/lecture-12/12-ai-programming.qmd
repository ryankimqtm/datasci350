...
format:
  clean-revealjs:
    self-contained: true
    footer: "[AI & Prompt Engineering](https://danilofreire.github.io/datasci350/lectures/lecture-12/12-ai-programming.html)"
transition: slide
...
- Temperature, and the settings the chat apps hide from you

[3. System prompts]{.alert}

- The instructions you never see, and what they do to the answers
:::

:::{.column width="50%"}
[4. Prompting techniques]{.alert}
...
- Prompt injection, and why an agent with your inbox is a risk

[6. Where it breaks]{.alert}

- Hallucination, bias, and performance that is uneven for no visible reason
:::
:::
:::
...
| GPT-5.5 | 272,000 tokens | 128,000 |
| Gemini 3.5 Flash | 1,000,000 tokens | 65,000 |
...
The same job on GPT-5.5 costs about [six times more]{.alert}, for work that a cheap model probably handles well
:::
...
- The chat apps [hide all of this]{.alert}. The developer playgrounds do not: try [Google AI Studio](https://aistudio.google.com/) or [OpenAI's playground](https://platform.openai.com/playground)
:::
...
:::{.columns}
:::{.column width="55%"}
Today was about [how to talk to models]{.alert}, and what an agent is in principle.

In [lecture 14]{.alert} you do it for real:

- Install a [coding agent]{.alert} in your terminal and let it read, edit, and run files in a project folder
- Call a model [from your own Python script]{.alert}, with your own API key
- Run a model [on your own laptop]{.alert}, with the internet switched off
- Work out which of those fits a given job, and when an agent should not be trusted with it
...
4. Rewrite the prompt with all four parts of PTCF. Name a reader, a task, some context, and an exact format
5. Run your version at temperature 0
6. Count how many of your format instructions the model actually followed
:::
...
- At 0.9 they will not be, and neither is more correct than the other
- The vague prompt gives you an essay. Yours should give you something you could paste into a document
- Step 6 is the real test. If the model ignored an instruction, [it was probably ambiguous]{.alert} rather than disobeyed
:::

:::{style="background: rgba(230, 57, 70, 0.1); padding: 15px; border-radius: 10px; font-size: 19px; margin-top: 15px;"}
Bring your rewritten prompt to the next class. We compare a few of them, and the disagreements are the interesting part
:::
:::
:::
:::
...
:::{.column width="50%"}
- After the quiz, [lecture 14]{.alert} takes today's material to the keyboard: coding agents, the API, and a model running on your own machine
