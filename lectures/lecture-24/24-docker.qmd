...
:::{.columns}
:::{.column width="50%"}
- Last time, we created a simple container with Docker
- We wrote a short [Dockerfile](https://docs.docker.com/engine/reference/builder/) with a [Python image](https://hub.docker.com/_/python) and a few packages, ran the container, and [uploaded it to Docker Hub](https://hub.docker.com/r/danilofreire/datasci350-example)
- Today, we will build a container from scratch with [all the tools from this course]{.alert}: `bash`, `git`, `Python`, `Jupyter`, `Quarto`, and `SQL`
- We will go through the main Dockerfile instructions: `FROM`, `LABEL`, `RUN`, `SHELL`, `ENV`, `EXPOSE`, and `CMD`
- We will install system packages with `apt`, Python libraries with `pip`, and Quarto with `wget`
- We will learn how to debug dependency errors, mount volumes to persist data, and manage containers (stop, remove, push)
- We will also discuss deploying containers to AWS
- If time allows, we will have Q&A about your projects (more time in our next class too)
:::
...
- That includes `bash`, `git`, `Python`, `Jupyter`, `Quarto`, `SQL`, and `Jupyter` tools
...
:::{style="margin-top: 30px; font-size: 22px; text-align: center;"}
[![](figures/docker-website.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/docker-website.png"}
:::
...
:::{style="margin-top: 30px; font-size: 26px;"}
- `RUN` executes commands in a new layer on top of the current image and [commits the results](https://docs.docker.com/engine/reference/commandline/commit/). Commits in Docker work like commits in Git, but they save changes to an image instead of a file system
- Remember `apt`? We will use `apt-get update` (once) and `apt-get install <package>` to install software, just as we did on AWS
  - Example: `apt-get update && apt-get install git -y`
- `--no-install-recommends` avoids installing unnecessary packages; `-y` auto-answers "yes" to prompts
- To keep the image small, we run `apt-get clean` and `rm -rf /var/lib/apt/lists/*` after installing, which removes the package cache
...
- The `-c` flag tells `bash` to read commands from a string, not from a file or standard input
...
:::{style="margin-top: 30px; font-size: 22px;"}
- We will also install [Quarto](https://quarto.org/), but Quarto is not on `pip` or `apt`, so we download the binary from <https://quarto.org/docs/get-started/>
- [`wget`](https://www.gnu.org/software/wget/) is a command-line tool for downloading files from the web, available on Ubuntu, macOS, and Windows
- We download the `.deb` file (Ubuntu's package format) and install it with `apt-get install`
- The `./` prefix tells `apt-get` to install the local file, not a package from the repositories
- `rm` removes the downloaded file after installation to keep the image smaller
...
- Finally, we run the Jupyter notebook server in the container on port `8888`, so we expose this port with `EXPOSE`
- The new JupyterLab comes with an embedded terminal, so you can run `bash` inside the JupyterLab interface and access all the tools we installed (`git`, `sqlite3`, `Quarto`) 😉
- We also create a working directory to persist notebooks outside the container
...
:::{style="margin-top: 30px; font-size: 22px; text-align: center;"}
[![](figures/docker04.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/docker04.png"}
:::
