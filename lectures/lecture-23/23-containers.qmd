...
:::{.columns}
:::{.column width="50%"}
- The [reproducibility crisis]{.alert}: many researchers cannot replicate published results, including their own
- Affects computer science, statistics, psychology, medicine, and many other fields
- Causes include p-hacking, publication bias, and low statistical power, but also [poor documentation and dependency management]{.alert}
- The statistical problems are harder to fix, but the computational side [can be solved with tools we already have]{.alert}
- A [Nature survey](https://www.nature.com/articles/533452a) found that 70%+ of scholars failed to reproduce someone else's work, and [over half could not reproduce their own]{.alert} 😳
- 90% of researchers agree there is a crisis
:::

:::{.column width="50%"}
- Missing code, undeclared packages, and OS differences are among the most common computational culprits

:::{style="text-align: center; margin-top: -20px;"}
[![](figures/reproducibility.webp){width="80%"}](#){data-modal-type="image" data-modal-url="figures/reproducibility.webp"}
:::
:::
:::
:::
...
[![](figures/trade-off.png){width="80%"}](#){data-modal-type="image" data-modal-url="figures/trade-off.png"}
:::
...
- `environment.yml` (for `conda`)
  - Creates an environment with specific packages and versions
  - Generate with `conda env export -f environment.yml`
  - `-f` is a flag for "file"
  - More details in the [conda documentation](https://docs.conda.io/projects/conda/en/latest/user-guide/tasks/manage-environments.html)

- `requirements.txt` (for `pip`)
  - Generate with `pip freeze | grep -v ‘file://’ > requirements.txt`
  - Install with `pip install -r requirements.txt`
  - `-r` is short for `--requirement` (read packages from a file)
...
- Three main concepts:
  - [Container]{.alert}: an executable package with everything needed to run an application
  - [Image]{.alert}: a snapshot that produces a container when started with `run` ([more here](https://stackoverflow.com/questions/23735149/what-is-the-difference-between-a-docker-image-and-a-container))
  - [Registry]{.alert}: a collection of repositories from which you pull images
...
:::{.column width="50%"}
- The syntax: `FROM` sets the base image, `RUN` executes commands during the build, and `CMD` runs when the container starts
