...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/scraping-record-anatomy.png}
\caption{The two questions answered on books.toscrape.com. Find the repeating branch, then find the fields inside it.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/scraping-inspector.png}
\caption{Step 3 in progress, in Chrome's inspector. Highlighting \texttt{article.product\_pod} in the tree lights up exactly one card and no more, so that is the branch holding one record. Its children name the fields: \texttt{h3 > a} is the title, \texttt{product\_price} the price.}
\end{figure}
...
```{=latex}
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{3.6cm}>{\raggedright\arraybackslash}p{7.2cm}>{\raggedright\arraybackslash}p{4.6cm}@{}}
\toprule
Selector & Reads as & Matches in our example \\
\midrule
\texttt{div} & every \texttt{div} element & both books \\
\texttt{.book} & every element with class \texttt{book} & both books \\
\texttt{\#b1} & the element with id \texttt{b1} & the first book only \\
\texttt{div.book} & \texttt{div} elements that also have class \texttt{book} & both books \\
\texttt{div.book h3} & any \texttt{h3} \emph{inside} a \texttt{div.book} & both titles \\
\texttt{h3 > a} & an \texttt{a} that is a \emph{direct child} of an \texttt{h3} & both links \\
\texttt{a[href]} & \texttt{a} elements that have an \texttt{href} attribute & both links \\
\texttt{p[class=\symbol{34}intro\symbol{34}]} & \texttt{p} whose class is exactly \texttt{intro} & the intro paragraph \\
\bottomrule
\end{longtable}
```
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{images/scraping-network.png}
\caption{An undocumented API, found in about thirty seconds. The page rendered the Marilyn Monroe quote from this JSON, so fetching this URL directly gives you the data already structured.}
\end{figure}
...
```{=latex}
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{5.9cm}>{\raggedright\arraybackslash}p{4.1cm}>{\raggedright\arraybackslash}p{5.4cm}@{}}
\toprule
Situation & Reach for & Why \\
\midrule
A documented API exists & \texttt{requests} + JSON & Stable, fast, explicitly permitted \\
No documented API, but the Network tab shows JSON & \texttt{requests} on that URL & An undocumented API still beats HTML \\
The data is in an HTML \texttt{<table>} & \texttt{pd.read\_html} & One line, then clean \\
...
Personal data, or \texttt{robots.txt} says no, or the ToS forbids it & Stop & Ask, or find another source \\
\bottomrule
\end{longtable}
```
...
```{=latex}
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{4.2cm}>{\raggedright\arraybackslash}p{4.7cm}>{\raggedright\arraybackslash}p{6.5cm}@{}}
\toprule
Goal & CSS selector (\texttt{select}) & \texttt{find\_all} equivalent \\
\midrule
Every paragraph & \texttt{p} & \texttt{find\_all(\symbol{34}p\symbol{34})} \\
Class \texttt{price} & \texttt{.price} & \texttt{find\_all(class\_=\symbol{34}price\symbol{34})} \\
Id \texttt{main} & \texttt{\#main} & \texttt{find(id=\symbol{34}main\symbol{34})} \\
\texttt{div} with class \texttt{book} & \texttt{div.book} & \texttt{find\_all(\symbol{34}div\symbol{34}, class\_=\symbol{34}book\symbol{34})} \\
Two classes at once & \texttt{p.instock.availability} & \texttt{find\_all(\symbol{34}p\symbol{34}, class\_=[\symbol{34}instock\symbol{34},} \newline \texttt{\symbol{34}availability\symbol{34}])} \\
Descendant, any depth & \texttt{div.book a} & chained \texttt{find\_all} \\
Direct child only & \texttt{ul > li} & \texttt{find\_all(\symbol{34}li\symbol{34}, recursive=False)} \\
Has an attribute & \texttt{a[href]} & \texttt{find\_all(\symbol{34}a\symbol{34}, href=True)} \\
Attribute equals & \texttt{a[href=\symbol{34}/b1\symbol{34}]} & \texttt{find\_all(\symbol{34}a\symbol{34}, href=\symbol{34}/b1\symbol{34})} \\
Attribute starts with & \texttt{a[href\^{}=\symbol{34}/books\symbol{34}]} & needs a regex \\
Attribute contains & \texttt{a[href*=\symbol{34}page\symbol{34}]} & needs a regex \\
First match only & \texttt{select\_one(...)} & \texttt{find(...)} \\
\bottomrule
\end{longtable}
```
...
```{=latex}
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{6.5cm}>{\raggedright\arraybackslash}p{8.5cm}@{}}
\toprule
Goal & Code \\
\midrule
Text inside an element & \texttt{el.get\_text(strip=True)} \\
An attribute, safely & \texttt{el.get(\symbol{34}href\symbol{34})} \\
An attribute, strictly & \texttt{el[\symbol{34}href\symbol{34}]} (raises if missing) \\
The class list & \texttt{el.get(\symbol{34}class\symbol{34})} returns a list \\
Readable HTML, for debugging & \texttt{el.prettify()} \\
\bottomrule
\end{longtable}
```
