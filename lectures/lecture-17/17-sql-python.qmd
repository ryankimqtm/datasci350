...
subtitle: Lecture 17 - SQLite and Python
author:
  - name: Danilo Freire
...
format:
  clean-revealjs:
    self-contained: true
    code-overflow: wrap
    footer: "[SQLite and Python](https://raw.githack.com/danilofreire/datasci350/main/lectures/lecture-17/17-sql-python.html)"
transition: slide
...
db_file = 'lecture17.db' # Database filename
...
- [`executemany()`]{.alert} lets you insert many rows at once: you write the SQL template once and pass a [list of tuples]{.alert} with the data
  - A [tuple]{.alert} is like a list, but uses parentheses instead of brackets and [cannot be changed]{.alert} after creation
- The `?` symbols are [placeholders]{.alert}: Python fills them in with each tuple's values
- Python's `None` is automatically converted to SQL's `NULL`
- As before, `conn.commit()` makes the insertions permanent
...
drivers = [
    ...
    ("Fernando Alonso",  "Aston Martin",      None,        None),
    ("Charles Leclerc",  "Ferrari",           "Monégasque", None),
]
...
- This works because `cur.execute()` [returns the cursor itself]{.alert}, and cursors are iterable
- You can write it all in one line:

```{python}
#| echo: true
#| eval: true
for row in cur.execute("SELECT driver_name, team FROM drivers WHERE victories > 50"): print(row)
```
:::
...
for row in cur.execute(query_in): print(row)
```
...
for row in cur.execute(query_not_in): print(row)
```
...
for row in cur.execute(query_between): print(row)
```
...
#| echo: true
#| eval: true
# Drivers with victories outside the 10-60 range
for row in cur.execute("SELECT driver_name, victories FROM drivers WHERE victories NOT BETWEEN 10 AND 60"): print(row)
```
:::
...
- `LIKE` enables simple pattern matching in text strings
  - [`%`]{.alert}: Matches any sequence (including none) of characters (like `*` in Bash)
  - [`_`]{.alert}: Matches exactly one character (like `?` in Bash)

```{python}
#| echo: true
#| eval: true
# Starts with 'L'
for row in cur.execute("SELECT driver_name FROM drivers WHERE driver_name LIKE 'L%'"): print(row)
```
...
#| eval: true
# Ends with 'Racing'
for row in cur.execute("SELECT team FROM drivers WHERE team LIKE '%Racing'"): print(row)
```
...
for row in cur.execute("SELECT driver_name FROM drivers WHERE driver_name LIKE '_a%'"): print(row)
```
:::
...
for row in cur.execute("SELECT driver_name FROM drivers WHERE LOWER(driver_name) LIKE 'l%'"): print(row)
```

- You can also use [`COLLATE NOCASE`]{.alert} to make a specific comparison case-insensitive
  - `COLLATE` tells SQLite [which rules to use when comparing text]{.alert}. `NOCASE` means "treat upper- and lower-case letters as the same"
  - Unlike `LOWER()`, this works directly on the original column without transforming it, which is cleaner and faster on large tables

```{python}
#| echo: true
#| eval: true
# Case-insensitive search for names starting with 'l' using COLLATE
for row in cur.execute('''SELECT driver_name FROM drivers 
                          WHERE driver_name LIKE 'l%' COLLATE NOCASE'''): print(row)
```
...
#| echo: true
#| eval: true
# Drivers whose names do NOT start with 'L'
for row in cur.execute("SELECT driver_name FROM drivers WHERE driver_name NOT LIKE 'L%'"): print(row)
```
:::
...
## Finding `NULL`s (`IS NULL`)  and `COALESCE`
...
#| echo: true
#| eval: true
for row in cur.execute("SELECT driver_name, victories FROM drivers WHERE victories IS NULL"): print(row)
```
...
for row in cur.execute('''SELECT driver_name, 
                          COALESCE(victories, 0) AS victories_filled FROM drivers'''): print(row)
```
:::
...
FROM drivers; 
''' 
for row in cur.execute(query_coalesce_sub): print(row)
```
...
- Necessary for ranking, running totals, moving averages, etc
...
if cur.fetchone()[0] == 0: # Not found
    cur.executemany('INSERT INTO drivers (driver_name, team, nationality, victories) VALUES (?, ?, ?, ?)', 
                    more_drivers_data)
    conn.commit()
...
print(f"SQLite Version: {cur.execute('SELECT sqlite_version();').fetchone()[0]}")
```
:::
...
print([desc[0] for desc in cur.description])  # Shows column names
...
- `--` is used for comments in SQL, so we can add notes about what each function does
...
for row in cur.execute(query_str2): print(row)
```
:::
...
for row in cur.execute(query_case1): print(row)
```
:::
...
for row in cur.execute(query_case2): print(row)
```
:::
...
for row in cur.execute(query_case_null): print(row)
```
:::
...
for row in cur.execute("SELECT * FROM drivers WHERE driver_name LIKE 'M%';"): print(row)

# Find 7 char nationalities
print("\n--- Nationalities with 7 chars ---")
for row in cur.execute("SELECT * FROM drivers WHERE LENGTH(nationality) = 7;"): print(row)

# Find L or M names
print("\n--- Drivers starting with L or M ---")
for row in cur.execute("SELECT * FROM drivers WHERE driver_name LIKE 'L%' OR driver_name LIKE 'M%';"): print(row)

# Find 1-10 wins
print("\n--- Drivers with 1-10 wins ---")
for row in cur.execute("SELECT * FROM drivers WHERE victories BETWEEN 1 AND 10;"): print(row)
...
for row in cur.execute(query_rank_nationality): print(row)
...
for row in cur.execute(query_driver_level): print(row)
...
:::{style="margin-top: 50px; font-size: 22px;"}
- **Option 1 (Easiest)**: [Delete the database file]{.alert}. Find `.db` file (`lecture17.db`) or use the following code:
...
db = Path('lecture17.db')
if db.exists():
    db.unlink()  # Ensure connection is closed first!
    print(f"Deleted: {db}")
```
...
with sqlite3.connect('lecture17.db') as conn:
